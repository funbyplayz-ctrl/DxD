...
if [ -z "$choice" ]; then
echo "=================================="
echo " QEMU VM Launcher (Root/Root Edition)"
echo "=================================="
...
# Configures cloud-init to unlock root and set password to 'root'
...
# Added genisoimage here to allow seed.iso creation for Debian
apt-get update && apt-get install -y qemu-system-x86 qemu-utils wget genisoimage
mkdir -p "$VM_DIR"
